...
public final class AdminLinearViewModel extends LinearViewModel {

    public AdminLinearViewModel(Context context) {
        super(context);
    }
    ...
    public View renderChildMap(final HashMap<String, String> hashMap) {
        ...
        ((TextView) innerView).setText(context.getResources().getString(R.string.bookedBy));
        innerView = view.findViewById(R.id.roomDescription);
        ((TextView) innerView).setText(hashMap.get(RoomBookingJSONHelper.RoomInfo.ROOM_BOOKED_BY));
        return view;
    }
